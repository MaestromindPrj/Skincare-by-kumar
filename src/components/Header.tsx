...
export const Header: React.FC = () => {
  ...
  // Smart autohide navbar: hide on scroll down, show on scroll up
  const updateVisibility = useCallback(() => {
    ...
    if (currentScrollY <= 10 || mobileMenuOpen) {
      setIsVisible(true);
    } else if (currentScrollY > prevScrollY + 5 && currentScrollY > 80) {
      setIsVisible(false);
  ...
  return (
    <>
      {/* Fixed header */}
      <header
        className={`fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-b border-[rgba(15,15,15,0.06)] transition-transform duration-300 ease-in-out ${
